...
# written by roamingnode@gmail.com

from pandas import read_csv  # Чтение DataFrame из файлов csv
...
def getBarsFromFile(fileName: str):
    """Получение бар из файла экспорта Finam"""

    # <TICKER>,<PER>,<DATE>,<TIME>,<OPEN>,<HIGH>,<LOW>,<CLOSE>,<VOL>
    # BR,1,20100111,103000,81.0000000,81.5000000,81.0000000,81.5000000,2

    df = read_csv(fileName,  # Получаем данные из файла
                  sep=',',  # Разделителем служит запятая
                  ...
                  # Для внутридневных данных дату и время закрытия бара WL хранит в 2-х колонках:
                  ...
                  converters={"<VOL>": convert_volume}  # убрать разделители и конвертировать в int
                  )
...
# взято за основу из https://smart-lab.ru/blog/514941.php
# переделано под requests добавлена опция записи в файл
# добавлены фьючерсы
def getBarsFromUrl(ticker: str, period: str, start_date: datetime, end_date: datetime, save_to=''):
    ...
    import finam.ticker_data as finam
    stocks = finam.getBarsFromUrl('SBER', 'daily', datetime.datetime(2020, 1, 1), datetime.datetime(2020, 11, 24), saveFile=True)
    futures = finam.getBarsFromUrl('SPFB.SBRF', 'hour', datetime.datetime(2017, 1, 1), datetime.datetime(2017, 12, 31), saveFile=True)
    ...
    futures = {'SPFB.GAZR': 17451, 'SPFB.BR': 22460, 'SPFB.ED': 21989, 'SPFB.Eu': 22010, 'SPFB.RTS': 17455,
               ...
               'SPFB.MAGN': 874042, 'SPFB.ALRS': 461013
               }

    FINAM_URL = "http://export.finam.ru/"  # сервер, на который стучимся
    if ticker.startswith('SPFB.'):  # check if ticker is futures ?
        market = 14
        em = futures[ticker]
    else:
        market = 0  # Для акций.
    ...
    payload = {
        ...
        # В каком формате брать даты. Выбор из 5 возможных.
    ...
    df = read_csv(io.StringIO(txt.content.decode('utf-8')),
                  ...
                  # WL хранит в 2-х колонках: Date и Time. Нужно их объединить в одну колонку DateTime
                  parse_dates={'DateTime': ['<DATE>', '<TIME>']},
                  dayfirst=True,  # В Windows с русской локализацией в дате сначала идет день, затем месяц
                  index_col='DateTime',  # Объединенная колонка DateTime будет индексом полученного DataFrame
                  decimal='.',  # В Finam десятичная часть разделяется точкой
    ...
    df.rename(columns={'<OPEN>': 'Open', '<HIGH>': 'High', '<LOW>': 'Low', '<CLOSE>': 'Close', '<VOL>': 'Volume'},
              inplace=True)
    if save_to != '':
        # задаём путь куда запишем котировки.
        # newline='\n' для Windows для других платформ возможно нужно удалить
        # with open(os.path.join(save_to, f"{ticker}_{period}_{start_date_rev}-{end_date_rev}.csv"), "w", newline='\n') as local_file:
        #    local_file.write(txt.content.strip().decode("utf-8") + '\n')
        if not os.path.exists(save_to):
            os.mkdir(save_to)
        df.to_csv(os.path.join(save_to, f"{ticker}_{period}_{start_date_rev}-{end_date_rev}.csv"), sep=';',
                  encoding='utf-8')
    return df
...
def to_backtrader_data(ticker, mode, from_date, to_date, path_save='ticker_data'):
    ...
    filename = f"{ticker}_{mode}_{from_date.strftime('%Y%m%d')}-{to_date.strftime('%Y%m%d')}.csv"
    if not os.path.isfile(os.path.join(path_save, filename)):
        bars = getBarsFromUrl(ticker, mode, from_date, to_date, save_to=path_save)

    # convert ticker_data to GenericCSVData class requirements
